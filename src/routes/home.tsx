import { A, createAsync, revalidate, useNavigate, useSubmission, type RouteDefinition } from "@solidjs/router";
import { createEffect, For, Show } from "solid-js";
import { getHomeDataQuery } from "~/api/home/home.actions";
import { logoutAction } from "~/api/users/users.actions";
...
export default function HomePage() {
  ...
  const data = createAsync(() => getHomeDataQuery());
  const logoutSub = useSubmission(logoutAction);
  const nav = useNavigate();

  createEffect(() => {
    if (logoutSub.result) {
      revalidate(getHomeDataQuery.key);
      console.log("logout");
      nav("/");
    }
  });
    ...
    <div class="flex flex-col">
      ...
      <div class="flex justify-end">
        <form action={logoutAction} method="post">
          <button class="btn btn-square" aria-label="logout" title="Logout">
